...
class Getnews:
    ...
    def getContent(self):
        ...
        del_complain = re.compile(r'<blockquote class="fin_reference cjbgtclass">.*?</blockquote>',re.S)
        del_href = re.compile(r'<(.*?)>',re.S)
        # 将正文写入文件
        for i in content:
            i = re.sub(del_complain,"",i)
            i = re.sub(del_href,"",i)
            i = re.sub(r'\s','',i)
...
if __name__ == "__main__":    
    baseURL = 'http://finance.sina.com.cn/stock/hkstock/ggscyd/2016-03-31/doc-ifxqxcnr5025439.shtml'
    #ls = Getnews(baseURL)
