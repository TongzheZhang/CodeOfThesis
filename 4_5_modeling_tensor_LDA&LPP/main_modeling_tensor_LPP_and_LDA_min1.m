...
total_num = length(y_min1);
train_num = ceil(total_num*0.8);
test_num = total_num - train_num;
y_class = zeros(1,total_num);
y_test_real = y_min1(train_num+1:total_num)';
for i = 1:total_num
    if y_min1(i)>=0.01
        y_class(i) = 1;
    elseif y_min1(i)<=-0.01
        y_class(i) = 2;
    else
        y_class(i) = 3;
    end
...
dim_t3=4;
%% 构建张量流
[re_tensor_flow, tensor_flow] = con_tensor_flow_min1(total_num,dim1,dim2,dim3,'outproduct','no');% 使用三个棍构建法或者默认的外积方法，是否归一化
%% 训练得到X1，X2，X3
num_class = 3;%分为几类
alpha = 1;
beta = 0.2;% 0<beta<1
[re_LPPandLDA_tensor_flow,T1new,T2new,T3new] = re_LPP_and_LDA_tensor_tucker(tensor_flow,y_min1,y_class,total_num,train_num,test_num,dim1,dim2,dim3,dim_t1,dim_t2,dim_t3,alpha,beta,num_class);
